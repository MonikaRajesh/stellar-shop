...
export async function signInWithOAuthProvider(provider: 'google' | 'linkedin_oidc' | 'github' | 'azure') {
  if (String(import.meta.env.VITE_ENABLE_GOOGLE_OAUTH || '').toLowerCase() !== 'true') {
    return { data: null, error: new Error('Social sign-in is not enabled for this deployment. Use email and password, or enable Google OAuth in Supabase first.'), skipped: true };
  }
  if (provider !== 'google') {
    return { data: null, error: new Error('Only Google sign-in is enabled in this deployment.'), skipped: true };
  }
  if (!supabaseEnabled || !supabase) return { data: null, error: new Error('Supabase authentication is not configured.'), skipped: true };
  const configured = String(import.meta.env.VITE_SITE_URL || '').trim().replace(/\/$/, '');
  const redirectTo = configured && !/localhost|127\.0\.0\.1/i.test(configured)
    ? configured
    : (window.location.origin.includes('localhost') ? window.location.origin : PRODUCTION_SITE_URL);
