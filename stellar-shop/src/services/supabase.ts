...
export async function signUpWithSupabase(
  ...
  const result = await supabase.auth.signUp({
    ...
    options: {
      data: metadata,
      emailRedirectTo: `${(import.meta.env.VITE_SITE_URL as string | undefined) || window.location.origin}/verify-email`,
...
export async function fetchCurrentProfile(userId: string) {
  ...
  return await supabase.from('profiles').select('id,name,email,mobile,role').eq('id', userId).maybeSingle();
}
