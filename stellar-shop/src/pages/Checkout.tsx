...
export function Checkout() {
  ...
  const saveSuccessfulOrder = async (paymentMethod: string, paymentId?: string, paymentOrderId?: string) => {
    ...
    const order = {
      ...
      address: `${address.name}, ${address.line}, ${address.city}, ${address.phone}`,
      paymentMethod,
  ...
  const placeOrderHandler = async () => {
    ...
    try {
      ...
      const razorpay = new window.Razorpay({
        ...
        notes: { address: `${address.line}, ${address.city}` },
        theme: { color: '#315efb' },
        config: {
          display: {
            blocks: {
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-3">
          ...
          <section className="rounded-card border border-base bg-elevated p-5">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              {addresses.map((a) => <button key={a.id} onClick={() => setAddressId(a.id)} className={classNames('rounded-xl border-2 p-4 text-left transition', addressId === a.id ? 'border-stellar-500 bg-stellar-500/5' : 'border-base hover:border-stellar-300')}>
                <div className="flex items-center justify-between"><span className="text-sm font-bold">{a.name}</span><span className="rounded-full bg-soft px-2 py-0.5 text-xs text-muted">{a.tag}</span></div>
                <p className="mt-1 text-sm text-muted">{a.line}</p><p className="text-sm text-muted">{a.city}</p><p className="mt-1 text-xs text-muted">{a.phone}</p>
        ...
        <div className="lg:sticky lg:top-24 lg:self-start">
          <div className="rounded-card border border-base bg-elevated p-5">
            ...
            <ul className="mt-4 space-y-3">{items.map((i) => <li key={i.productId} className="flex gap-3"><img src={i.product.images[0]} alt={i.product.name} className="h-14 w-14 rounded-lg object-cover" /><div className="flex-1"><p className="line-clamp-1 text-sm font-semibold">{i.product.name}</p><p className="text-xs text-muted">Qty {i.quantity}</p><p className="text-sm font-bold">{formatINR(i.product.price * i.quantity)}</p></div></li>)}</ul>
