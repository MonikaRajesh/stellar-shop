...
import { createRemoteOrder } from '@/services/supabase';
...
export function Checkout() {
  ...
  const placeOrderHandler = async () => {
    ...
    try {
      if (isSupabaseConfigured) await createRemoteOrder(order);
      dispatch(placeOrder(order));
      if (!buyNowItem) dispatch(clearCart());
      dispatch(addToast({ message: isSupabaseConfigured ? 'Order placed and saved securely!' : 'Order placed successfully!', type: 'success' }));
      navigate('/orders');
    } catch (error: any) {
      dispatch(addToast({ message: error?.message || 'Could not place the order.', type: 'error' }));
    } finally {
      setPlacing(false);
    }
  };
