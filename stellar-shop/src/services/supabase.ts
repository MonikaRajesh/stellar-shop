...
export async function sendLoginOtp(identifier: string, mode: 'email' | 'phone', captchaToken?: string) {
  if (!supabaseEnabled || !supabase) return { data: null, error: new Error('Supabase authentication is not configured.'), skipped: true };
  const value = identifier.trim();
  const result = mode === 'email'
    ? await supabase.auth.signInWithOtp({ email: value, options: { shouldCreateUser: false, ...(captchaToken ? { captchaToken } : {}) } })
    : await supabase.auth.signInWithOtp({ phone: value, options: { shouldCreateUser: false, ...(captchaToken ? { captchaToken } : {}) } });
  return { ...result, skipped: false };
}

export async function verifyLoginOtp(identifier: string, token: string, mode: 'email' | 'phone') {
  if (!supabaseEnabled || !supabase) return { data: null, error: new Error('Supabase authentication is not configured.'), skipped: true };
  const value = identifier.trim();
  const result = mode === 'email'
    ? await supabase.auth.verifyOtp({ email: value, token: token.trim(), type: 'email' })
    : await supabase.auth.verifyOtp({ phone: value, token: token.trim(), type: 'sms' });
  return { ...result, skipped: false };
}
