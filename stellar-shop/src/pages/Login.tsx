    ...
    <div className="mx-auto flex max-w-md flex-col px-4 py-12 lg:px-6">
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="rounded-card border border-base bg-elevated p-8 shadow-soft">
        <div className="text-center">
          ...
          <p className="mt-1 text-sm text-muted">Sign in with your social account or email and password.</p>
        </div>

        <div className="mt-6 grid grid-cols-2 gap-2">
          {enabledSocialProviders.includes('google') && <Button type="button" variant="outline" disabled={busy} onClick={() => void socialLogin('google')} className="w-full justify-center gap-2"><FaGoogle /> Google</Button>}
          {enabledSocialProviders.includes('linkedin_oidc') && <Button type="button" variant="outline" disabled={busy} onClick={() => void socialLogin('linkedin_oidc')} className="w-full justify-center gap-2"><FaLinkedinIn /> LinkedIn</Button>}
          {enabledSocialProviders.includes('github') && <Button type="button" variant="outline" disabled={busy} onClick={() => void socialLogin('github')} className="w-full justify-center gap-2"><FaGithub /> GitHub</Button>}
          {enabledSocialProviders.includes('azure') && <Button type="button" variant="outline" disabled={busy} onClick={() => void socialLogin('azure')} className="w-full justify-center gap-2"><FaMicrosoft /> Microsoft</Button>}
        </div>

        <div className="my-6 flex items-center gap-3 text-xs text-muted"><span className="h-px flex-1 bg-base" /><span>OR CONTINUE WITH EMAIL</span><span className="h-px flex-1 bg-base" /></div>
        ...
        <div className="mt-5 rounded-xl bg-soft p-3 text-xs text-muted"><p className="flex items-center gap-2 font-semibold text-base"><ShieldCheck className="h-4 w-4" /> Secure social authentication</p><p className="mt-1">Enabled social sign-in providers are handled securely by Supabase OAuth. No OTP is used.</p></div>
