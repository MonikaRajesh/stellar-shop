...
export function Register() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setBusy(true);
    if (supabaseEnabled) {
      const result = await signUpWithSupabase(form.email, form.password, { name: form.name, mobile: form.mobile });
      if (result.error || !result.data?.user) {
        dispatch(addToast({ message: result.error?.message || 'Unable to create account', type: 'error' }));
        setBusy(false);
        return;
      }
      if (!result.data.session) {
        dispatch(addToast({ message: 'Account created. Check your email to confirm, then sign in.', type: 'success' }));
        navigate('/login');
        setBusy(false);
        return;
      }
      const u = result.data.user;
      dispatch(loginSuccess({ user: { id: u.id, name: form.name, email: form.email, mobile: form.mobile }, token: result.data.session.access_token }));
  ...
  return (
    <div className="mx-auto flex max-w-md flex-col px-4 py-12 lg:px-6">
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="rounded-card border border-base bg-elevated p-8 shadow-soft">
        ...
        <p className="mt-5 text-center text-sm text-muted">{supabaseEnabled ? 'Supabase authentication enabled.' : 'Demo mode — no real authentication is performed.'}</p>
